...
def send_activation_email(request, email, code):
    context = {
        ...
        'uri': request.build_absolute_uri(reverse('login_register:activate', kwargs={'code': code})),
    }
...
def send_activation_change_email(request, email, code):
    context = {
        ...
        'uri': request.build_absolute_uri(reverse('login_register:change_email_activation', kwargs={'code': code})),
    }
...
def send_reset_password_email(request, email, token, uid):
    context = {
        ...
        'uri': request.build_absolute_uri(
            reverse('login_register:restore_password_confirm', kwargs={'uidb64': uid, 'token': token})),
    }
